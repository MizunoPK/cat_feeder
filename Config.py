...
class Config:
    ...
    SERVO_GPIO_SLOTS = [21, 24] # The gpio slots being used by the servos - should correspond to the cat order
    SERVO_SIDES = ["L", "R"]
    ...
    ULTRASONIC_ECHO_PINS = [18, 12] # The gpio slots being used by the ultrasonic echo pins - should correspond to the cat order
    ULTRASONIC_MAX_DISTANCE = 25 # Maximum distance we check for - measured in cm
    ULTRASONIC_COOLDOWN = 15 # seconds until we've decided the cat has definitely left

    # --- BUTTON CONFIG ---
    BUTTON_PIN = [5,17] # GPIO pin for button
    LED_PIN = [6,27] # GPIO pin for LED